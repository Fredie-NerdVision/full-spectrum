<?php
/**
 * Spam defences for the booking form.
 *
 ...
 * misjudged booking enquiry can be recovered from the admin rather than lost.
 *
 * @package mrh
 */

const MRH_SPAM_MIN_SECONDS = 4;
const MRH_SPAM_MAX_SECONDS = 6 * HOUR_IN_SECONDS;
const MRH_SPAM_THRESHOLD   = 4;
const MRH_SPAM_SCORE_META  = '_mrh_spam_score';
const MRH_SPAM_REASON_META = '_mrh_spam_reasons';

/**
 * Sales pitches that arrive through a public booking form. Each costs half the
 * quarantine threshold on its own, so one phrase plus a link is enough and a
 * phrase in an otherwise ordinary enquiry is not.
 *
 * @return string[]
 */
function mrh_spam_phrases() {
	return array(
 ...
 */
function mrh_spam_timestamp_field() {
	$now = time();

	printf(
		'<input type="hidden" name="mrh_started" value="%s">',
		esc_attr( $now . '|' . wp_hash( $now . '|mrh_booking' ) )
	);
}
 ...
 * @return int|false
 */
function mrh_spam_token_age( $token ) {
	...
	if ( ! hash_equals( wp_hash( $parts[0] . '|mrh_booking' ), $parts[1] ) ) {
		return false;
	}
 ...
 * @return int
 */
function mrh_spam_link_count( $text ) {
	preg_match_all(
 ...
 */
function mrh_spam_score( $values, $age ) {
	$score   = 0;
	$reasons = array();
	...
	/* A venue, a school name or a person's name never contains a link. */
	foreach ( array( 'name', 'organization', 'venue' ) as $field ) {
		$value = isset( $values[ $field ] ) ? $values[ $field ] : '';

		if ( '' !== $value && ( mrh_spam_link_count( $value ) > 0 || str_contains( $value, '@' ) ) ) {
			$score    += 4;
	...
	foreach ( mrh_spam_phrases() as $phrase ) {
 ...
 */
function mrh_spam_turnstile_enabled() {
	return defined( 'MRH_TURNSTILE_SITE_KEY' ) && MRH_TURNSTILE_SITE_KEY
		&& defined( 'MRH_TURNSTILE_SECRET_KEY' ) && MRH_TURNSTILE_SECRET_KEY;
}

function mrh_spam_turnstile_field() {
	if ( ! mrh_spam_turnstile_enabled() ) {
		return;
	}
	...
	printf(
		...
		esc_attr( MRH_TURNSTILE_SITE_KEY )
	);
}
 ...
 */
function mrh_spam_turnstile_passes( $token ) {
	if ( ! mrh_spam_turnstile_enabled() ) {
		return true;
	}
	...
	$response = wp_remote_post(
		...
		array(
			'timeout' => 10,
			'body'    => array(
				'secret'   => MRH_TURNSTILE_SECRET_KEY,
				'response' => $token,
 ...
 */
add_filter(
	'manage_' . MRH_ENQUIRY_POST_TYPE . '_posts_columns',
	function ( $columns ) {
		$columns['mrh_spam'] = __( 'Spam check', 'mrh' );
...
add_action(
	'manage_' . MRH_ENQUIRY_POST_TYPE . '_posts_custom_column',
	function ( $column, $post_id ) {
		if ( 'mrh_spam' !== $column ) {
			return;
		}

		$score = get_post_meta( $post_id, MRH_SPAM_SCORE_META, true );

		if ( '' === $score || (int) $score < MRH_SPAM_THRESHOLD ) {
			esc_html_e( 'Clean', 'mrh' );
		...
		printf(
			...
			esc_html__( 'Held as spam', 'mrh' ),
			esc_html( (string) get_post_meta( $post_id, MRH_SPAM_REASON_META, true ) )
		);
	},
	10,
	2
